...
public class SmtpServer implements Runnable {
    ...
    public void run() {
        ...
        try {
            ...
            logger.error("Server loop failure", e);
        } finally {
            ready = false;
    ...
    private void initializeServerSocket() throws Exception {
        logger.debug("Initializing SMTP socket on port " + port);
        serverSocket = new ServerSocket(port);
        logger.debug("Dummy SMTP started on port " + port);
    }
    ...
    private void serverLoop() throws IOException {
        ...
        while (!isStopped()) {
            ...
            logger.debug("New client connection: " + clientSocket.getRemoteSocketAddress());
