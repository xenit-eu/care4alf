...
care4alf.controller('locks', ($scope,$http: ng.IHttpService) => {
    var self = this;

    $scope.locks = [];

    this.getData = function () {
        $http.get("locks/list").success(function(data){$scope.locks = data;});
    };

    this.getData();

    $scope.unlock = function(noderef)
    {

        $http.get("locks/unlock",{'params':{'noderef':noderef}}).success(function(data){
            this.getData();

        }).error(function(){alert('Error while unlocking!')});
